...
namespace Shop.Common.RabbitMq
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            ...
            Func<Task> handle, Func<TMessage, ShopException, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            Func<Task> handle, Func<TMessage, ShopException, IRejectedEvent> onError = null)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (exception is ShopException ShopException && onError != null)
                {
                    var rejectedEvent = onError(message, ShopException);
